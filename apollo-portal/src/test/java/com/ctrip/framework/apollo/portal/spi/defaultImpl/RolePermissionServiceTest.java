...
public class RolePermissionServiceTest extends AbstractIntegrationTest {
	...
	public void testCreatePermissions() throws Exception {
		...
		Set<Permission> created = rolePermissionService
				.createPermissions(Sets.newHashSet(somePermission, anotherPermission));

		Set<Long> permissionIds = created.stream().map(Permission::getId).collect(Collectors.toSet());
	...
	public void testCreateRoleWithPermissions() throws Exception {
		...
		List<RolePermission> rolePermissions = rolePermissionRepository
				.findByRoleIdIn(Sets.newHashSet(createdFromDB.getId()));
	...
	public void testUserHasPermission() throws Exception {
		...
		assertFalse(
				rolePermissionService.userHasPermission(someUserWithNoPermission, somePermissionType, someTargetId));
		assertFalse(rolePermissionService.userHasPermission(someUserWithNoPermission, anotherPermissionType,
				anotherTargetId));
