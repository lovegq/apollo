...
public class NamespaceService {
	...
	public NamespaceService(final ReleaseHistoryService releaseHistoryService,
			final NamespaceRepository namespaceRepository, final AuditService auditService,
			final @Lazy AppNamespaceService appNamespaceService, final MessageSender messageSender,
			final @Lazy ItemService itemService, final CommitService commitService,
			final @Lazy ReleaseService releaseService, final @Lazy ClusterService clusterService,
			final @Lazy NamespaceBranchService namespaceBranchService, final NamespaceLockService namespaceLockService,
	...
	public Namespace findChildNamespace(String appId, String parentClusterName, String namespaceName) {
		...
		// the child namespace is the intersection of the child clusters and child
		// namespaces
		for (Namespace namespace : namespaces) {
	...
	public Namespace deleteNamespace(Namespace namespace, String operator) {
		...
		// Child namespace releases should retain as long as the parent namespace
		// exists, because parent namespaces' release
		// histories need them
		if (!isChildNamespace(namespace)) {
		...
		if (childNamespace != null) {
			namespaceBranchService.deleteBranch(appId, clusterName, namespaceName, childNamespace.getClusterName(),
					NamespaceBranchStatus.DELETED, operator);
			// delete child namespace's releases. Notice: delete child namespace will not
			// delete child namespace's releases
		...
		namespace.setDeleted(Boolean.FALSE);
		...
		// Publish release message to do some clean up in config service, such as
		// updating the cache
	...
	public Namespace update(Namespace namespace) {
		Namespace managedNamespace = namespaceRepository.findByAppIdAndClusterNameAndNamespaceName(namespace.getAppId(),
